...
const closePopup = async(req, res) => {
    const showPopup = false;
    const updateComment = await User.findByIdAndUpdate({ _id: req.body.id }, { $set: { comments: req.body.comment, testStatus: req.body.testStatus } });
    const appointments = await User.find({
        appointmentID: { $ne: null },
    }).populate("appointmentID");
    res.render('examiner', { appointments, showPopup });
...
const getDataByTestType = async(req, res) => {
    const showPopup = false;
    const appointments = await User.find({ testType: req.body.type }).populate("appointmentID");
    res.render('examiner', { appointments, showPopup });
}

module.exports = { redirectToEmployer, getUserDetails, closePopup, getDataByTestType };